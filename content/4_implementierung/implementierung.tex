	...
	\subsection{Target}\label{ss:demo_target}
		...
		\subsubsection*{Zustandsmaschine}
			...
			\begin{itemize}
				...
				\item \textbf{Software Taster} Erfolgt die Bedienung entfernt vom Messaufbau via \gls{g:host}, so bietet die \textit{RT Main}-\gls{ac:vi} einen entsprechenden Taster.
			\end{itemize}
			%
...
\section{Vergleich Konzept - Implementierung}
	...
	\subsection*{Schätzung Rotationswinkel und Rotation}
		Für das Überprüfen der Rotationswinkel und der anschliessenden Rotation wird eine statische Messung in z-Position (erläutert in Abschnitt \vref{ss:methode_tran}, Testbeschreibung) durchgeführt. Die Kontrolle beschränkt sich auf das Endergebnis der Rotation, das $a_{Soll}=[0\quad0\quad 9.81]^T$ sein sollte.\par
		%
		Wie erwartet liefert das Konzept ein ähnliches Resultat von $a_{Konzept} = [-0.004\quad0.004\quad9.971]^T$. Unter den gleichen Bedingungen errechnet die Implementierung einen Beschleunigungsvektor von $a_{Implementation}=[0.008\quad0.013\quad9.934]^T$. Vergleicht man die beiden Resultate, kann man auf eine korrekte Implementierung des Konzepts schliessen. Die geringen Abweichungen könnten darin begründet sein, dass die Auflösung der verwendeten Datentypen sich in \gls{g:matlab} und \gls{g:labview} unterscheiden. 
	%	
