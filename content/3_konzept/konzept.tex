...
Das Konzept kann teilweise aus dem später beschriebenen Beispielprojekt in Abschnitt \vref{subsec:myposture} extrahiert werden. Dort wurden eine ähnliche Aufgabenstellung gelöst. Nach der Literaturstudie und der Absprache mit den Betreuern haben wir uns entschieden die Methode mit der doppelten Integration der Beschleunigung der Bewegung umzusetzen, wie in der Literaturstudie in \cite{lit:doubleint} gesehen.\par
%
Die Beschleunigung der Bewegung wird von einem auf dem Knie montierten drei Achsen Beschleunigungssensor aufgezeichnet. Da der Sensor beliebig im Raum platziert ist, enthalten die aufgezeichneten Beschleunigungsdaten zum einen die Erdbeschleunigung zum andern die Bewegungsbeschleunigung. In unserem Falle ist nur die Beschleunigung der Bewegung von Interesse. Somit muss die Erdbeschleunigung detektiert und diese von der Bewegungsbeschleunigung getrennt werden. Dies erreicht man durch die Kompensation der Orientierung des Sensors. Die statische Erdbeschleunigung kann dafür gebraucht werden. Diese zeigt stets in Richtung der z-Achse unseres Koordinatensystem des Betrachters.  Die zu detektierende Bewegung wir nur in einer Ebene betrachtet. In diesem Falle in der x-y-Ebene. Senkrecht zu dieser Ebene steht die statische Erdbeschleunigung. Die Beschleunigungskomponenten der x- und y-Achse sind noch zweimal zu integrieren, um auf die Bewegung schliessen zu können. Das Blockdiagramm des Konzeptes ist in Abbildung \vref{img:blockdiagramm} ersichtlich. Auf die einzelnen Blöcke soll in den folgenden Unterkapiteln näher eingegangen werden.
%
% Blockdiagramm
...
%
Die Erdbeschleunigung ist in jedem Falle eine konstante Beschleunigung. Aus diesem Grund kann diese Beschleunigung als eine reine DC-Komponente betrachtet werden. Mit einer Tiefpass-Filterung wird somit einfach die Erdbeschleunigung von der Beschleunigung der Bewegung getrennt. Als Tiefpassfilter wurde ein \todo{Überprüfen ob Filterwahl gut} \gls{ac:fir}-Filter gewählt. 
%
% Filterentwurf
...
Um die Bewegung des Sensors im Koordinatensystem des Betrachters darstellen zu können, muss die Orientierung des Sensors bekannt sein. Als Bezugssystem dient die Erde, bei welcher die z-Achse des Koordinatensystems parallel zur Erdbeschleunigung liegt. Mit dem Gravitätsvektor $\overrightarrow{a_g}$ und der gemessenen Erdbeschleunigung des Sensors $\overrightarrow{a_{sens}}$ kann mit einfachen, grundlegenden Vektoroperationen auf den Drehwinkel $\Phi$ sowie auf die Richtung der Rotationsachse $\overrightarrow{n}$ zwischen den beiden Vektoren geschlossen werden. Dies ist in Abbildung \vref{img:Vektor} illustriert. Mit dem Drehwinkel $\Phi$ und der Rotationsachse $\overrightarrow{n}$ kann die im Kapitel \vref{ch:grundlagen} in Abschnitt \vref{sub:rot} beschriebene Rotationsmatrix aufgestellt werden.  
%
\image{content/3_konzept/image/Vektor_bearb}{scale=0.7}{htbp}[Rotation][img:Vektor]

%
\subsubsection*{Betrachtungssystem}
Um die Orientierung des Sensors zu kompensieren, ist zum Überblick in Abbildung \vref{img:koordinatensystem} das Koordinatensystem des Betrachters sowie das Koordinatensystem des myRIO 1900 aufgezeigt. Das Koordinatensystem des Betrachters liegt mit der z-Achse parallel zur Erdbeschleunigung. Bei frontaler Betrachtung resultieren \gls{g:transversal}e Bewegungen nach links und rechts durch eine Abnahme bzw. Zunahme der y-Komponente. Hingegen bewirken \gls{g:sagital}e Bewegungen nach vorne und hinten eine Zu- bzw. Abnahme der x-Komponente.

\begin{figure}[htbp]
        \centering
        \begin{subfigure}[htbp]{0.3\textwidth}
                \includegraphics[width=\textwidth]{content/3_konzept/image/koordinatensystem_betrachter}
                \caption{Koordinatensystem des Betrachters}
                \label{img:koord_betrachter}
        \end{subfigure}%
        \quad \quad
        \begin{subfigure}[htbp]{0.3\textwidth}
                \includegraphics[width=\textwidth]{content/3_konzept/image/koordinatensystem_myRIO}
                \caption{Koordinatensystem \gls{g:myrio}}
                \label{img:koord_myRIO}
        \end{subfigure}        
        \caption{Koordinatensystem}\label{img:koordinatensystem}
\end{figure}    
%
Das myRIO 1900 hat eine beliebige Ausrichtung im Raum. Durch die Montage des Beschleunigungssensors sind die Achsen wie in Abbildung \vref{img:koord_myRIO} ersichtlich gegeben.
%
\subsubsection*{Drehwinkel}
Der Drehwinkel wie in Abbildung \vref{img:Vektor} gesehen entspricht gerade dem Zwischenwinkel der beiden Vektoren $\overrightarrow{a\low{g}}$ sowie den Beschleunigungsdaten $\overrightarrow{a\low{sens}}$. Der Zwischenwinkel zweier Vektoren wird über das Skalarprodukt berechnet. Nach Papula \cite{lit:papula} ist dies wie in Gleichung \vref{eq:dotp} dargestellt möglich.
%
\formula{
...
Um die Beschleunigung der Bewegung zu erhalten, können mehrere Ansätze verfolgt werden. Zum einen kann die berechnete Erdbeschleunigung am Sensor von der gemessenen Beschleunigung am Sensor subtrahiert werden. Die gemessene Beschleunigung am Sensor beinhaltet bekanntlich die Erdbeschleunigung sowie die benötigte Beschleunigung der Bewegung. Ein weiterer Ansatz ist auf die gemessenen Beschleunigungsdaten zusätzliche eine Hochpassfilterung anzuwenden. Hiermit wird gerade das Gegenteil zur Tiefpassfilterung erreicht. Mit der Hochpassfilterung werden die Beschleunigungsdaten von der Erdbeschleunigung gekapselt. Die Komponenten der Bewegungsbeschleunigung können somit mit einem Hochpass mit tief angelegter Durchlassfrequenz ermittelt werden. Da sich die Autoren nach Absprache mit den Betreuern nur auf die Bewegung in einer Ebene beschränken ist es ausreichen nur eine Rotation in das Betrachtungssystem vorzunehmen. Dabei ist die Bewegung auf der x- und y-Achse auszuwerten. Auf der z-Achse ist die Erdbeschleunigung vorhanden. \par
%
...
% 
Mit der Rotationsmatrix und der berechneten Beschleunigung der Bewegung kann die Transformation ins Koordinatensystem des Betrachters mit einer einfachen Multiplikation erreicht werden. Der Output ist dabei die Beschleunigung der Bewegung im definierten Betrachtungssystem. Die Orientierung des Beschleunigungssensors am Knie, die beliebig ist, ist somit kompensiert.
%
% Bewegungsdetektion
...
Um von der Beschleunigung auf die Position zu schliessen, kann die Beschleunigung bekannterweise durch zweimaliges integrieren nach der Zeit berechnet werden. Integriert man die Beschleunigung einmal, so erhalten wir die Geschwindigkeit.
%
\formula{
v(t) = \int a(t)dt + v_0 
}{
...
\section{Verifikation Schätzung Rotationswinkel und Rotation}\label{s:verifikation_rotation}
	...
	\subsection{Material/Testumgebung}
		...
		%
		Für die Messung wird die Befestigung und der Bewegungsfreiraum des Testaufbaus so eingestellt, dass ein möglichster grosser Winkelbereich zur Verfügung steht (insgesamt \unit[56]{°}). Dazu wird der Rotationsarm sowie die Querverbindung wie in Abbildung  \vref{img:messaufbau_rot} gezeigt angeordnet. Die Position des \gls{g:myrio} M\low{1}, das für die Aufzeichnung der Beschleunigungsdaten eingesetzt wird, befindet sich am äussersten Ende des Schwenkarm.\par
		%
	...
	\subsection{Methoden}
		...
		\subsubsection*{Testgruppe Messmethodik}
			Die Messmethodik der Testgruppe basiert zum einen auf den aufgezeichneten Beschleunigungsdaten und zum anderen aus mathematischen Operationen\footnote{siehe Abschnitt \vref{s:blockdiagramm} für den verwendeten Algorithmus}, die die Winkelschätzung vornehmen. Dazu dient das \gls{g:matlab}-Skript \texttt{XXX}, das im Anhang \vref{s:anhang_matlab} zu finden ist.
		%
		...
		\subsubsection*{Testbeschreibungen}
			...
			\begin{enumerate}
				...
				\item \gls{g:labview}-Programm starten. Es müssen während  \unit[6]{s} Daten aufgezeichnet werden\footnote{Bedingt durch die Filterlänge des Tiefpassfilters, siehe Abschnitt XXX}.
		...
		\subsubsection*{Datenauswertung}
			...
			%
			Die absolute Messabweichung, gemittelt\footnote{Es wird der arithmetische Mittelwert verwendet} über die fünf Messungen,  beträgt bei den unterschiedlichen Winkeln \unit[-2.28]{°} (\unit[62]{°}), \unit[-1.65]{°} (\unit[77]{°}), \unit[-1.45]{°} (\unit[90]{°}), \unit[-0.22]{°} (\unit[105]{°}) und \unit[0.74]{°} (\unit[118]{°}). Das entspricht einem mittleren absoluten Fehler von \unit[-0.92]{°} über alle Messungen hin betrachtet.\par
			%
			Der relative Messabweichungen, wiederum gemittelt, bemessen sich auf \unit[-0.038]{\%} (\unit[62]{°}), \unit[-0.021]{\%} (\unit[77]{°}), \unit[-0.016]{\%} (\unit[90]{°}), \unit[-0.002]{\%} (\unit[105]{°}) und \unit[0.006]{\%} (\unit[118]{°}). Dies ergibt einen mittleren relativen Fehler von \unit[-0.014]{\%} über alle Messungen hinweg.\par
			%
			Analysiert man die Messabweichung hinsichtlich ihrer Standardabweichung, so ergibt dies für die absoluten Messungen \unit[0.036]{°} und für die relativen \unit[0.0003]{\%}.\par 
			%
	...
	%
	%
	\subsection{Interpretation}\todo{Sind die Messungen OK?}
%
%
...
\section{Verifikation Translation}\label{s:verifikation_translation}
	...
	\subsection{Methoden}
		...
		\subsubsection*{Testbeschreibungen}
			...
			\end{itemize}\par 
			%
			Die zu fahrende Distanz wird auf \unit[5]{cm} festgelegt. Dies entspricht dem festgelegten Messbereich, der in Abschnitt \vref{ss:messbereich} erläutert ist. Bedingt durch die manuelle Durchführung der Translation kann wird die Toleranz bei der Messung auf $pm$\unit[5]{cm} festgelegt.\par
			%
			...
			\begin{enumerate}
				...
				\item Messung starten
					\begin{enumerate}
						...
						\item Translation durchführen (langsam oder schnell).
...
%				\end{figure}    
			...
			\paragraph*{y-Achsen Position}
				Befindet sich das \gls{g:myrio} in der y-Achsen Position, so ergibt sich über 15 Messungen ein mittlerer absoluter Fehler von \unit[1.338]{cm} (relativ \unit[26.8]{\%}). Dies bei der Geschwindigkeit langsam. Die Analyse hinsichtlich des gemessenen Mittelwertes und Standardabweichung ergibt \unit[6.338]{cm} und \unit[1.355]{cm}. Die Abbildung \vref{img:y_tran_langsam_y} zeigt die 15 Messungen mit eingezeichneten Kenngrössen (blau Mittelwert der Messung und rot die Standardabweichung).
				%
				\image{content/3_konzept/image/mittel_5_Y_Y}{scale=0.6}{htbp}[y-Position Translation langsam][img:y_tran_langsam_y]
				%
				Ein Erhöhung der Geschwindigkeit bringt einen mittleren absoluten Messfehler von \unit[1.360]{cm} und ein relativen von \unit[27.2]{\%} zu Tage. Dies bei einem ermittelten Mittelwert von \unit[6.360]{cm} und einer Standardabweichung von \unit[0.697]{cm}, wie die Abbildung \vref{img:y_tran_schnell_y}.
				%
			...
			\paragraph*{z-Achsen Position}
				...
				%
				Bei der Geschwindigkeit schnell erhöht sich wiederum der absolute Fehler auf \unit[0.769]{cm} (entspricht relativ \unit[15.4]{\%}). Die Messung an sich hat einen Mittelwert von \unit[-4.231]{cm} bei einer Standardabweichung von \unit[0.255]{cm}. Die Abbildung \vref{img:z_tran_schnell_y} zeigt das gemessene Ergebnis.\par 
				%
				\image{content/3_konzept/image/schnell_5_Z_Y}{scale=0.6}{htbp}[z-Position Translation schnell][img:z_tran_schnell_y]
			%
			Die Messerergebnisse zusammengefasst sind in der Tabelle XXX dargestellt. Darauf basierend erfolgt nun im folgenden Abschnitt \vref{ss:interpretation_tran} die Interpretation und Diskussion.
			%
...
\section{Verifikation Translation und Rotation überlagert}\label{s:verifikation_transaltion_rotation}
	...
	\subsection{Grund/Zweck der Messung}\todo{anpassen an Abschnitt 3.1}
		...
		\begin{hypo}\label{hypo:tran_rot}
			Das Messergebnis ist eine Bewegung in der \gls{g:transversalebene}. Sie ist dabei eine Widerspiegelung des  reale Ablaufs mit einer Abweichung von maximal \unit[5]{mm}.
		\end{hypo}
	%
	%
	...
	\subsection{Methoden}
		...
		\subsubsection*{Testbeschreibungen}\todo{Ablauf}
			...
			%
			Die Auslenkung $\alpha$ kann durch das Verstellen der Querstrebe (siehe Abbildung XXX) eingestellt werden. Die Position des \gls{g:myrio} wird dabei nicht verändert.\todo{Auslenkungen anpassen, da mechanisch nicht möglich}
			%
			\begin{itemize}
